...
const Profile_Setup_pg = () => {
  ...
  const Handle_User_Provider_Data = async () : Promise<void> => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/init/user/profile`, {withCredentials : true});
  ...
  const MutationHandler = async (form_Data : FormData) => {
    try {
      ...
      const resp = await axios.put(`${import.meta.env.VITE_BACKEND_URL}/user/profile/setup`, form_Data, {withCredentials : true});
  ...
  return (
    <div className=" flex bg-gray-100 flex-col gap-10 items-center h-fit p-3 pb-20">
      ...
      method='PUT' 
      action={`${import.meta.env.VITE_BACKEND_URL}/user/profile/setup`} 
